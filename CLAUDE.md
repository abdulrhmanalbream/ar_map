...
`GlassesDisplay.kt` + `ui/GlassesHud.kt`: when a USB-C display (XREAL One Pro or similar glasses) is plugged in while `ArNavActivity` is foreground, a `Presentation` replaces the default screen mirroring with a landscape HUD (arrow, distance, Arabic instruction) fed by the same `CampusState`. Two constraints shape it: pure black background because black pixels are invisible on see-through glasses (never add filled panels there), and the arrow follows the *phone's* compass — the glasses expose no sensors. Requires Samsung screen-mirroring mode, not DeX (DeX owns the external display; apps can't present to it).
