...
export default function LocationMap({
  ...
  useEffect(() => {
    ...
    const resize = new ResizeObserver(() => {
      if (map.current === instance) instance.invalidateSize({ animate: false });
    });
  ...
  useEffect(() => {
    ...
    for (const device of devices) {
      ...
      points.push(point);
      const stale =
        ageMs(location.recordedAt) > LOCATION_STALE_MS || isOffline(device);
      const state = needsHelp(device) ? "help" : stale ? "stale" : "active";
      ...
      if (
        ...
        L.circle(point, {
          radius: Math.min(location.accuracyM, 100_000),
          color: "#18675a",
          weight: 1,
          fillOpacity: 0.08,
          interactive: false,
  ...
  const fitAll = () => {
    const points = located.map((device) => {
      const p = sharedLocation(device)!;
  ...
  return (
    <section className="map-panel" aria-label="خريطة آخر المواقع المشتركة">
      <div className="map-heading">
        <div>
          <MapPin size={18} />
          <h2>المتابعة على الخريطة</h2>
          <span className="subtle">{number(located.length)} موقع مشترك</span>
        </div>
        <button
          className="icon-button"
          title="إظهار كل المواقع"
          aria-label="إظهار كل المواقع"
          onClick={fitAll}
      ...
      <div className="map-canvas">
        ...
        {!located.length && (
          <div className="map-empty">
            ...
            <strong>بانتظار المواقع المشتركة</strong>
            <p>
              تظهر المواقع هنا عندما يفعّل الأعضاء مشاركة الموقع من تطبيق المطوف الذكي.
            </p>
          </div>
        )}
        {tileError && (
          <div className="map-error" role="status">
            تعذّر تحميل بعض أجزاء الخريطة.
            <button
              onClick={() => {
                setTileError(false);
      ...
      <div className="map-footer">
        ...
        <span>آخر موقع فقط · الدائرة تمثل دقة الموقع</span>
      </div>
    </section>
  );
}
